...
这一小节只需要读懂，不要求马上熟练换算，也不影响后面的课程。

我们平时使用十进制。十进制的一位可以写 `0` 到 `9`，向左一位表示的大小变成原来的 10 倍。例如：

```text
365 = 3 × 100 + 6 × 10 + 5 × 1
```

计算机里的电子开关更容易稳定地区分两种状态，就像“关”和“开”。我们用 `0` 和 `1` 表示这两种状态，这就是**二进制**。
...
```text
1101（二进制）= 8 + 4 + 0 + 1 = 13（十进制）
```

二进制从右向左，每一位依次代表 `1、2、4、8、16……`，每向左一位就变成原来的 2 倍。

#### `bit` 和 `byte`

- 一个 `0` 或 `1` 所占的位置叫作一个**二进制位**，英文是 `bit`，读作“比特”；
- 在目前常见的计算机中，8 个 `bit` 组成 1 个 `byte`，中文叫“字节”；
- 常见竞赛环境中的 `int` 通常使用 32 个 `bit`，`long long` 通常使用 64 个 `bit`。

现阶段不需要背负数在二进制中怎样保存。只要理解：保存数据的位置数量有限，所以一种数据类型能表示的数也有范围；位数更多，通常就能表示更大的范围。

#### 动手点亮小灯

先不要写程序，照着 `8、4、2、1` 四盏灯手算：

1. `1010`（二进制）表示十进制中的多少？
2. 十进制的 `6` 应该打开哪两盏灯？写成四位二进制是多少？
3. 1 个 `byte` 通常由多少个 `bit` 组成？

答案在本教程《练习答案与提示》的“第 2 章”中。以后学习位运算时，我们还会更系统地认识二进制。
...
- [ ] 我能看懂并使用 `x = x + 1`。
- [ ] 我知道整数类型有范围，过大会溢出。
- [ ] （选学）我知道二进制使用 `0` 和 `1`，也知道 `bit` 和 `byte` 的关系。
